...
//    FILE* src_file_fd;
    ...
    long file_size1=file_size(command->arguments[1]);
    //convert the long to string -needed for the concat
    char* file_size_str= malloc(21);
    if(sprintf(file_size_str,"%lu",file_size1)<0){
        printf("error in casting the file_size!");
        send(client->sock_fd,"nok file", sizeof("nok file"),0); //send ok_msg to client
        FREE(file_size_str);
        return -1;
    }
    char* ok_msg=concat("ok ",file_size_str);//now command holds the name of command.tmp
    printf("file_size_str: %s\n",file_size_str);
    send(client->sock_fd,ok_msg, sizeof(ok_msg),0); //send ok_msg to client
    FREE(file_size_str);
    FREE(ok_msg);
    client->conn_state=DOWNLOADING;
    //TODO - maybe there is a need to change the send file name to ?.tmp
    int src_ptr=open(command->arguments[1],0); //pointer to the start of src file
    while(file_size1>0){//while there is a data to send
       /* if(file_size1-= sendfile(client->sock_fd, src_ptr, NULL, file_size1) < 0){
            send(client->sock_fd,"nok state", sizeof("nok state"),0);
        ...
        file_size1-= sendfile(client->sock_fd, src_ptr, NULL, 1024);
    }
    close(src_ptr); //close streaming
    recv(client->sock_fd,buffer,twoK,0);
    if(buffer[0]!='d' || buffer[1]!='o'|| buffer[2]!='n'|| buffer[3]!='e') {//if something else than "done" - send nok done and disconnect
        printf("download Error: %s\n",buffer+4);
        //disconnect user- mission requirement
        send(client->sock_fd,"nok done", sizeof("nok done"),0);
        FREE(client->client_id); //free the client name (malloced in conn)
        close(client->sock_fd); // close stream
        init_client(client,"nil"); //init the client
        return -1;
    }
    //else- got done from client
    client->conn_state=CONNECTED;
    send(client->sock_fd,"ok", sizeof("ok"),0);
    printf("Sent file %s\n",command->arguments[1]);
    return 0;
    ...
    while(cond){
        ...
        while(cond){ //after get a client - handle only him
            ...
            if(strstr(command->arguments[0],"ls")){
                ...
                if(strstr(command->arguments[0],"get")){
                flag=1;
                if(get(command)<0)
                    flag=2;
            }
